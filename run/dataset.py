...
class RegData(torch.utils.data.Dataset):
    ...
    def __init__(self, opts, crop=lambda x: x):
        ...
        self.vis_folder = os.path.join(opts.dataroot, 'src_v2r')
        self.ir_folder = os.path.join(opts.dataroot, 'src_r2v')

        self.vis_fake_folder = os.path.join(opts.dataroot, 'tar_r2v')
        self.ir_fake_folder = os.path.join(opts.dataroot, 'tar_v2r')
        self.size = 256
        self.crop = torchvision.transforms.RandomCrop(self.size)
        ...
        #
        # self.vis_fake_list = sorted(os.listdir(self.vis_fake_folder))
        # self.ir_fake_list = sorted(os.listdir(self.ir_fake_folder))
        #
        # self.vis_list = self.vis_list[:221] if not opts.debug else self.vis_list[:4]
        # self.ir_list = self.ir_list[:221] if not opts.debug else self.vis_list[:4]
        # self.vis_fake_list = self.vis_fake_list[:221] if not opts.debug else self.vis_list[:4]
        # self.ir_fake_list = self.ir_fake_list[:221] if not opts.debug else self.vis_list[:4]

        input_names = sorted(os.listdir(self.vis_folder))
        random.shuffle(input_names)
        self.vis_list = input_names[:221] if not opts.debug else input_names[:4]
        self.ir_list = input_names[:221] if not opts.debug else input_names[:4]
        self.vis_fake_list = input_names[:221] if not opts.debug else input_names[:4]
        self.ir_fake_list = input_names[:221] if not opts.debug else input_names[:4]

        print(len(self.vis_list), len(self.ir_list), len(self.vis_fake_list), len(self.ir_fake_list))
    ...
    def __getitem__(self, index):
        ...
        vis_fake_path = os.path.join(self.vis_fake_folder, self.vis_fake_list[index])
        ir_fake_path = os.path.join(self.ir_fake_folder, self.ir_fake_list[index])
        ...
        assert os.path.basename(vis_fake_path) == os.path.basename(
            ir_fake_path), f"Mismatch ir_fake:{os.path.basename(ir_fake_path)} vis_fake:{os.path.basename(vis_fake_path)}."
        ...
        vis_fake = self.imread(path=vis_fake_path, flags=cv2.IMREAD_GRAYSCALE)
        ir_fake = self.imread(path=ir_fake_path, flags=cv2.IMREAD_GRAYSCALE)

        vis_ir = torch.cat([vis, ir, vis_fake, ir_fake], dim=1)
        if vis_ir.shape[-1] <= self.size or vis_ir.shape[-2] <= self.size:
            vis_ir = TF.resize(vis_ir, self.size)
        ...
        vis, ir, vis_fake, ir_fake, vis_warped, ir_warped, vis_fake_warped, ir_fake_warped, disp = torch.split(patch,
                                                                                                               [3, 3, 3,
                                                                                                                3, 3, 3,
                                                                                                                3, 3,
                                                                                                                2],
        ...
        return ir, vis, ir_warped, vis_warped, disp, vis_fake, ir_fake
